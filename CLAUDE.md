...
> **Platform-image exception (step 15B):** The catalog rule is "every tool template points at a vendor-published image", one-and-only-one platform image (`executors/content-fetcher/`) exists to supply `/entry`. **Gatling** breaks this rule as of step 15B: `executors/gatling/Dockerfile` (eclipse-temurin + Maven Central bundle zip + a small `gatling-run` wrapper) is the SECOND platform image. Reason: Gatling 3.9.x's `gatling.sh` in LOCAL run mode returns exit=0 even when assertions fail (only Gatling Enterprise's `--wait` flag maps failures to exit), and no arm64-native community image existed at commit time. `gatling-run` reads `<results>/*/js/assertions.json` and exits non-zero on any `"result": false`, restoring "exit code honest" for the template. This exception stays confined to gatling; if a future tool hits the same wall the answer is another platform image (with the same file-header rationale + smoke evidence), not a runtime hack in the operator.
